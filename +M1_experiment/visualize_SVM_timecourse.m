...
for type_id = 1:length(type)
    for time_no = 1:length(time_type)
        ...
        for subj = 1:length(subjects)
            ...
            M1_experiment.visualise.plot.timecourse(times,score,subjects{subj},type{type_id},time_type{time_no},[output_path 'subjects/']);
            all_subjects_score = [all_subjects_score;score];
            all_score = [all_score;score_all.'];
        end
        M1_experiment.visualise.plot.all_timecourse(times,all_subjects_score,all_score,subjects,type{type_id},time_type{time_no},output_path); 
