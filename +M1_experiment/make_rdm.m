    ...
    for type_id = 1:length(type)
        for time_no = 1:length(time_type)
            for iter = 1:ex_cnt
                ...
                filename = [home_path subjects{subj} '/' type{type_id} '_' subjects{subj} '_' num2str(iter) '.mat'];
                load(filename);
            ...
            saveas(gcf,[home_path 'RDM_img/' subjects{subj} '_' type{type_id} '_' time_type{time_no} '.png']);
            
            plot(times,score_mean);
            title([type{type_id} '-' time_type{time_no} '-' num2str(subj) '-mean accuracy']);
            %saveas(gcf,['/home/nakajima/ドキュメント/研究室/移行用/20230622/' cate '_' cate2 '_rdm_timecorse_spearman.png']);
            saveas(gcf,[home_path 'RDM_img/' subjects{subj} '_' type{type_id} '_' time_type{time_no} '_mean_accuracy_timecorse.png']);
